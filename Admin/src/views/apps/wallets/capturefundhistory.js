...
class UsersList extends React.Component {
  static getDerivedStateFromProps(props, state) {
    let { capture_id, wallet_type} = queryString.parse(props.location.search) 
    if (
  ...
  state = {
    ...
    columnDefs: [
      ...
      },
      {
        ...
        cellRendererFramework: params => {
          return (
            <div className=""> {params.value ? params.value.email : ''}</div>
          )
        }
    },
    {
      ...
      cellRendererFramework: params => {
        return (
          <div className=""> {params.value ? params.value.first_name : ''} {params.value ? params.value.middle_name : ''} {params.value ? params.value.last_name : ''}</div>
        )
      }
    },
    {
      ...
      cellRendererFramework: params => {
        return (
          <div className=""> {params.value} {params.data.type ? " ("+params.data.type+")" : ''}</div>
        )
      }
    },
    {
    ...
    },
    {
      ...
      cellRendererFramework: params => {
        return (
          <div className=""> {typeof(params.value) == 'object' ? params.value.txid ? params.value.txid : params.value.transactionHash : params.value}</div>
        )
      }
    },
    {
  ...
  async componentDidMount() {
    ...
    const body = {
        ...
        capture_id : this.state.capture_id, 
        wallet_type : this.state.wallet_type, 
        action: 'all_captured'
      }
      postAPICall('user_wallet_capture', body).then((response) => {
