...
class UsersList extends React.Component {
          ...
          return (
            <div className="actions cursor-pointer">
              <Edit
                className="mr-50"
                size={28}
                onClick={() => history.push("./edit/UserEdit?user_id="+params?.data?.user_id+"&active_tab=2")}
              />
              <Trash2
                size={28}
                onClick={() => {
  ...
  async componentDidMount() {
    let params =  "?action=verified&raw=0&admin_user_id="+this.state.currentUserId+"&per_page="+this.state.pageSize+"&page=1";
          getAPICall('getbankkyc'+params)
    .then(response => {
    ...
    if (this.state.gridApi !== prevState.gridApi) {
      const dataSource = {
        getRows: (rowData) => {
          ...
          console.log(params);
          getAPICall("getbankkyc" + params).then((res) => {
  ...
  // async componentDidMount() {
  //   let params = "?action=verified&raw=0&admin_user_id="+this.state.currentUserId;
  //   getAPICall('getbankkyc'+params)
  //   .then(response => {
  //     const rowData = response.data;
  //     this.setState({ rowData });
            ...
            <CardBody style={{height: "85vh"}}>
              <div className="ag-theme-material ag-grid-table"  style={{height: "80vh"}}>
                <div className="ag-grid-actions d-flex justify-content-between flex-wrap mb-1" >
                  ...
                  <div className="">
                    <div className="h2 float-left">Total Bank Verified KYC : {this.state.rowData !== null ?  this.state.rowData.totalCount : 0 }</div>
                  </div>
                ...
                {this.state.rowData !== null ? (
                  ...
                  <>
                       <div className="float-left">
                        ...
                        <h2 className="float-left ml-1">System is Calculating All Bank KYC.</h2>
                  </>
                )}
              </div>
            </CardBody>
